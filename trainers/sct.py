...
#from utils.utils import compute_os_variance
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.sct.N_CTX
        ctx_init = cfg.TRAINER.sct.CTX_INIT
        ...
        self.class_token_position = cfg.TRAINER.sct.CLASS_TOKEN_POSITION
...
class sct(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.sct.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.sct.PREC == "fp32" or cfg.TRAINER.sct.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.sct.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.sct.PREC
    ...
    def test_ood(self, data_loader, T, id_flag):
        ...
        for batch_idx, (images, labels) in enumerate(tqdm(data_loader)):
            ...
            if id_flag:
                ...
                labels_all.extend(labels.float().cpu().numpy())
                predicted_all.extend(output.argmax(1).item())
            else:
                labels_all.extend(output.shape[1] * [output.shape[1]])
                

            output /= 100.0
            output_local /= 100.0
            smax_global = to_np(F.softmax(output / T, dim=-1))
            _, predicted_local = smax_global.argmax(dim=-1)
